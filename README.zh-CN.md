...
| --- | --- |
| `¥377.89` | 所选货币行的总余额 |
| `████░░░░` | 8 格用量条：有消耗速率时按跑道归一（满格约等于 12 小时跑道）；此前按余额对警告区间的比例 |
| `~` | 显示值为过期数据：上次刷新失败，保留旧值 |
| 颜色 | 有速率时按跑道（红 < 2 小时，黄 < 12 小时）；此前按绝对阈值（默认 ¥20 / ¥5） |
...
`~/.pi/agent/pi-deepseek-balance-snapshots.jsonl`。速率只在置信门控之后显示：
至少 3 个快照、跨度至少 1 小时、同一币种、且位于最近一次充值之后。未达门控时
不显示任何内容。该速率为账户级——包含共享此 key 的所有客户端的消耗，不只本
pi 会话。充值会重置窗口而不是产生负速率。跑道投影在后续版本提供；v0.1 先采集数据。
...
向下越限时提醒一次；充值后重新武装。默认 ¥20 警告、¥5 告急——这是对典型
消耗速度的猜测，故标注为默认值。用 `PI_DEEPSEEK_BALANCE_THRESHOLDS="20,5"`
覆盖（CNY；`0,0` 关闭）。
